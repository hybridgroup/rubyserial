...
class Serial
  def initialize(address, baude_rate=9600, data_bits=8)
    ...
    @error  = RubySerial::Win32.error_check
    @open = true

    RubySerial::Win32::DCB.new.tap do |p|
      p[:dcblength] = RubySerial::Win32::DCB::Sizeof
      RubySerial::Win32.GetCommState @serial, p
      p[:baudrate] = baude_rate
      p[:bytesize] = data_bits
      p[:stopbits] = RubySerial::Win32::DCB::ONESTOPBIT
      p[:parity]   = RubySerial::Win32::DCB::NOPARITY
